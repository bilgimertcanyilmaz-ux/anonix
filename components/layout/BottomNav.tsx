        ...
        {items.map(({ href, label, Icon, highlight, badge }) => (
          <li key={href} className="flex-1">
            <Link
              href={href}
              aria-label={label}
              className="flex flex-col items-center gap-1 text-[10px] font-medium text-slate-400 transition-colors hover:text-white"
            >
              {highlight ? (
                <span className="-mt-5 flex h-12 w-12 items-center justify-center rounded-full bg-brand-gradient shadow-glow ring-4 ring-ink-950/85">
